...
public class Movement : MonoBehaviour {
	...
	[SerializeField]
	private Sprite stillCrouch;

	/*
	[SerializeField]
	private GameObject shield;
	private float originalShieldY;
	private float lowerShieldY;
	*/
	...
	void Update ()
	{
		...
		if (_animator.GetCurrentAnimatorStateInfo (0).IsName ("Crouch")) {
			shield.transform.position = new Vector3 (shield.transform.position.x, lowerShieldY, shield.transform.position.z);
		}
		if (!_animator.GetCurrentAnimatorStateInfo (0).IsName ("Crouch")) {
			shield.transform.position = new Vector3 (shield.transform.position.x, lowerShieldY, shield.transform.position.z);
		}
